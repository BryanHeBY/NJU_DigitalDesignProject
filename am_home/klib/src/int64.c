...
{
    udwords udwords_a, udwords_b, udwords_res;
    udwords_a.all = a;
    udwords_b.all = b;
    udwords_res.all = 0;
    while(udwords_a.s.low || udwords_a.s.high) {
        if(udwords_a.s.low&1) udwords_res.all += udwords_b.all;
        udwords_b.s.high = (udwords_b.s.high<<1) | (udwords_b.s.low>>31);
        udwords_b.s.low <<= 1;
        udwords_a.s.low = ((udwords_a.s.high&1)<<31) | (udwords_a.s.low>>1);
        udwords_a.s.high >>= 1;
    }
    return udwords_res.all;
}
